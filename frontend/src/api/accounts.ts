...
export type AccountStatus =
  "offline" | "online" | "working" | "idle" | "restricted" | "error" | "banned";
export type AccountSpamCheckStatus =
  "unknown" | "queued" | "checking" | "clear" | "restricted" | "flagged" | "error";
export type AccountType = "promoter" | "guardian_bot";
export type AccountAssetTier =
  "unknown" | "month_1" | "month_3_6" | "year_1" | "year_2" | "year_3_plus";
export type AccountWarmupStage =
  "observe" | "seed" | "soft" | "ramp" | "normal" | "cooldown";
export type ProxyMode = "dynamic" | "static" | "none";
export type AccountRiskLevel =
  "normal" | "watch" | "limited" | "frozen" | "quarantined";
export type AccountOperationMode = "growth" | "ad_only";
export type AccountListPersonaSummary = Omit<
  AccountPersonaSummary,
  "effective_enabled"
> &
...
export interface Account {
  ...
  owned_group_created_count?: number;
  ...
  spam_check_status: AccountSpamCheckStatus;
  spam_checked_at?: string;
  spam_check_summary?: string;
  spam_restriction_confirmed_at?: string;
  restriction_source?: string;
  restriction_reason?: string;
  restriction_detected_at?: string;
  risk_score: number;
...
const normalizeAccount = (item: unknown): Account => {
  ...
  return {
    ...
    account_type:
      value.account_type === "guardian_bot" ? "guardian_bot" : "promoter",
    operation_mode: value.operation_mode === "ad_only" ? "ad_only" : "growth",
    owned_group_created_count: Number(value.owned_group_created_count ?? 0),
    asset_tier:
      typeof value.asset_tier === "string"
    ...
    warmup_stage:
      typeof value.warmup_stage === "string"
    ...
    status:
      typeof value.status === "string"
        ...
        : "offline",
    spam_check_status:
      typeof value.spam_check_status === "string"
        ? (value.spam_check_status as AccountSpamCheckStatus)
        : "unknown",
    spam_checked_at: optionalString(value.spam_checked_at),
    spam_check_summary: optionalString(value.spam_check_summary),
    spam_restriction_confirmed_at: optionalString(
      value.spam_restriction_confirmed_at,
    ),
    restriction_source: optionalString(value.restriction_source),
    restriction_reason: optionalString(value.restriction_reason),
    restriction_detected_at: optionalString(value.restriction_detected_at),
    risk_score: Number(value.risk_score ?? 0),
    risk_level:
      typeof value.risk_level === "string"
        ? (value.risk_level as AccountRiskLevel)
        : "normal",
    risk_pause_until: optionalString(value.risk_pause_until),
    risk_recovery_until: optionalString(value.risk_recovery_until),
    risk_reason: optionalString(value.risk_reason),
    ...
    proxy_mode:
      typeof value.proxy_mode === "string"
        ? (value.proxy_mode as ProxyMode)
        : "dynamic",
    static_proxy_id: value.static_proxy_id
      ? Number(value.static_proxy_id)
      : undefined,
...
export const accountsApi = {
  list: async (params?: AccountListParams): Promise<AccountListPayload> => {
    ...
    return {
      list: Array.isArray(response.data?.data)
        ? response.data.data.map(normalizeAccount)
        : [],
  ...
  connect: async (
    id: number,
  ): Promise<{ account_id: number; status: AccountStatus }> => {
  ...
  syncProfileBio: async (id: number, profileBio?: string): Promise<Account> => {
    const payload =
      profileBio === undefined ? undefined : { profile_bio: profileBio };
    const response = await apiClient.post(
      `/accounts/${id}/profile-bio/sync`,
      payload,
    );
    return normalizeAccount(response.data);
  },
  ...
  getRiskEvents: async (
    id: number,
    params?: { limit?: number },
  ): Promise<AccountRiskEventsPayload> => {
    const response = await apiClient.get(`/accounts/${id}/risk-events`, {
      params,
    });
    return response.data.data;
  },
  ...
  manualAdjustRisk: async (
    ...
    data: {
      score_delta?: number;
      set_score?: number;
      target_level?: AccountRiskLevel;
      clear_pause?: boolean;
      reason?: string;
    },
  ): Promise<AccountRiskSummary> => {
    const response = await apiClient.post(
      `/accounts/${id}/risk/manual-adjust`,
      data,
    );
    return response.data.data;
  },
  manualBan: async (id: number, reason = "manual_ban"): Promise<Account> => {
    const response = await apiClient.post(`/accounts/${id}/manual-ban`, {
      reason,
    });
    return normalizeAccount(response.data);
  },
...
export type ApiConfigPlatform = "windows" | "macos" | "android" | "ios" | "any";
...
export interface ApiConfig {
  ...
  platform: ApiConfigPlatform;
  account_count: number;
  explicit_account_count?: number;
  env_fallback_count?: number;
  under_cap?: boolean;
  created_at?: string;
  updated_at?: string;
}
...
export interface ApiConfigSummary {
  max_accounts_per_config: number;
  accounts_total: number;
  accounts_bound: number;
  accounts_unbound: number;
  accounts_env_fallback?: number;
  accounts_with_session: number;
}

export interface ApiConfigAssignResult {
  assigned: { account_id: number; os_type: string; api_config_name: string }[];
  needs_relogin: { account_id: number; os_type: string; api_config_name: string }[];
  failed: { account_id: number; error: string }[];
  summary: Partial<ApiConfigSummary>;
}

export const apiConfigsApi = {
  list: async (): Promise<{ configs: ApiConfig[]; summary?: ApiConfigSummary }> => {
    const response = await apiClient.get("/accounts/configs");
    return {
  ...
  create: async (data: {
    ...
    const response = await apiClient.post("/accounts/configs", data);
    return response.data;
  },

  delete: (name: string) => {
    return apiClient.delete(`/accounts/configs/${encodeURIComponent(name)}`);
  },

  assign: async (): Promise<ApiConfigAssignResult> => {
    const response = await apiClient.post("/accounts/configs/assign");
    return response.data.data;
  },
};
